...
describe SingaporeCPFCalculator::Year2015::SPR2FG::Age50To55ContributionCalculator do
  ...
  describe "#calculate" do
    ...
    context "when the total wages amounts to 749.99" do
      ...
      it { expect(result).to equal_cpf total: 232.00, employee: 112.00, employer: 120.00 }
    end

    context "when the total wages amounts to 750.00" do
      let(:ordinary_wages) { 750.00 }
      it { expect(result).to equal_cpf total: 233.00, employee: 112.00, employer: 121.00 }
    end

    context "when the total wages amounts to 5,000.00" do
      let(:ordinary_wages) { 5_000.00 }
      it { expect(result).to equal_cpf total: 1_550.00, employee: 750.00, employer: 800.00 }
    end
