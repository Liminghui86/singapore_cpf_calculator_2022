...
describe SingaporeCPFCalculator::Year2015::SPR1GG::Age50BelowContributionCalculator do
  ...
  describe "#calculate" do
    ...
    context "when the total wages amounts to 500.00" do
      ...
      it { expect(result).to eq "total" => 20.00, "employee" => 0.00, "employer" => 20.00 }
    end
    ...
    context "when the total wages amounts to 749.99" do
      ...
      it { expect(result).to eq "total" => 67.00, "employee" => 37.00, "employer" => 30.00 }
    end

    context "when the total wages amounts to 750.00" do
      let(:ordinary_wages) { 750.00 }
      it { expect(result).to eq "total" => 68.00, "employee" => 37.00, "employer" => 31.00 }
    end

    context "when the total wages amounts to 5,000.00" do
      let(:ordinary_wages) { 5_000.00 }
      it { expect(result).to eq "total" => 450.00, "employee" => 250.00, "employer" => 200.00 }
    end
