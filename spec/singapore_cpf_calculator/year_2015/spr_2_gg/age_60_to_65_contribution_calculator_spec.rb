...
describe SingaporeCPFCalculator::Year2015::SPR2GG::Age60To65ContributionCalculator do
  ...
  describe "#calculate" do
    ...
    context "when the total wages amounts to 749.99" do
      ...
      it { expect(result).to eq "total" => 82.00, "employee" => 56.00, "employer" => 26.00 }
    end

    context "when the total wages amounts to 750.00" do
      let(:ordinary_wages) { 750.00 }
      it { expect(result).to eq "total" => 83.00, "employee" => 56.00, "employer" => 27.00 }
    end

    context "when the total wages amounts to 5,000.00" do
      let(:ordinary_wages) { 5_000.00 }
      it { expect(result).to eq "total" => 550.00, "employee" => 375.00, "employer" => 175.00 }
    end
