...
describe SingaporeCPFCalculator::Year2014::SPR1FG::Age50BelowContributionCalculator do
  ...
  describe "#calculate" do
    ...
    context "when the total wages amounts to 749.99" do
      ...
      it { expect(result).to eq "total" => 157.00, "employee" => 37.00, "employer" => 120.00 }
    end

    context "when the total wages amounts to 750.00" do
      let(:ordinary_wages) { 750.00 }
      it { expect(result).to eq "total" => 158.00, "employee" => 37.00, "employer" => 121.00 }
    end
    ...
    context "when the total wages amounts to 5,000.00" do
      ...
      it { expect(result).to eq "total" => 1_050.00, "employee" => 250.00, "employer" => 800.00 }
    end
